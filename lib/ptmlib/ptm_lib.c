                       ...
                       int client_len)
{
    ...
    *cmd_id = atoi(hdr);
    ...
    for (i = j = 0; (i < csv_field_len(fld)) && (j < (client_len - 1)); i++) {
        if (!isspace(hdr[i])) {
            client_name[j] = hdr[i];
            j++;
        }
...
ptm_lib_find_key_in_msg(void *ctxt, const char *key, char *val)
{
    ...
    for(hstr = csv_field_iter(hrec, &hfld),
        ...
        if ((strlen(key) == csv_field_len(hfld)) &&
            !strncmp(hstr, key, csv_field_len(hfld))) {
    ...
    while (bytes_read != len) {
        ...
        if (rc <= 0) {
            if (errno && errno != EAGAIN) {
                ...
                if (bytes_read) {
                    DLOG("max retries - recv error(%s) bytes read %d(%d)\n",
